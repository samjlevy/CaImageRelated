...
for rs = 1:length(reg_paths)
    if ~exist(fullfile(reg_paths{rs},'RegisteredImageSL.mat'),'file')
        disp(['did not find image registration data for ' reg_paths{rs}])
        [~, ~, ~] = manual_reg_SL(base_path, reg_paths{rs});
    else 
        disp(['found registration for ' reg_paths{rs}])
    end
end
...
if any(cellfun(@(x) strcmpi(x,reg_path),fullReg.RegSessions,'UniformOutput',false))
    disp(['this session ' reg_path ' is already registered, skipping'])
else
    ...
    unmatchedBaseCells = allClosestCells(unpairedRegCells);
    ...
    while doneManual == 0
        ...
        switch forceAssign
            case 'Yes'
                ...
                imagesc(baseUnpaired); title('Base Session Masks')
                ...
                imagesc(regUnpaired); title('Reg Session Masks')
    ...
    if isempty(foundTwice) && assigned == numRegCells && max(matchedBaseCells) <= numBaseCells
        ...
        try
            ...
            save(fullfile(base_path,'fullReg.matROIavg'),'fullRegROIavg','-v7.3')
        catch
            keyboard
        end
    else
