function [TMap_unsmoothed, TMap_firesAtAll, TMap_zRates, OccMap, RunOccMap, xBin, TCounts] =...
    PFsLinTBTalternation(trialbytrial, binEdges, minspeed, saveName, smth,condPairs,dimUse)
%This is specialized to run with the double plus data format. 
%All TMaps get return in bins arranged from center outwards
%Do NOT use input parser, tends to be slow enough it will drag down a shuffle
...
if strcmpi(minspeed,'numFrames')
    ...
    for condI = 1:numConds
        switch dimUse
            ...
            case {'y','Y'}
                newTrialsX = cellfun(@(x) ceil([1:length(x)]/framesperbin),trialbytrial(condI).trialsY,'UniformOutput',false);
        end
