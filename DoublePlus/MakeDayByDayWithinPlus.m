function [daybyday, sortedSessionInds, useDataTable] = MakeDayByDayWithinPlus(mousePath, getFluoresence, deleteSilentCells)
...
load(fullfile(mousePath,'PlusMazeDataTable.mat'))

%Choose which files being included: these should have user input options
whichFilesUse = PlusMazeDataTable.HasPos == 1 & PlusMazeDataTable.HasFinalBeh;
...
for sessI = 1:height(useDataTable)
    ...
    daybyday.behavior{sessI}.TrialCorrect = lapParsed.Correct;
