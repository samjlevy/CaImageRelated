...
dnmpFolder = 'C:\Users\Sam\Desktop\DNMPfinalData';
ffFolder = 'C:\Users\Sam\Desktop\DNMPfinalData\ForcedUnforced';
...
%Rebuild cross tak registration
for mouseI = 1:numMice
    rrFile = fullfile(dnmpFolder,mice{mouseI},'reReg.mat');
    if exist(rrFile)==0
        [reReg] = ReconstructCrossTaskReg(fullfile(dnmpFolder,mice{mouseI}),...
                                          fullfile(dnmpFolder,mice{mouseI}),...
                                          fullfile(ffFolder,mice{mouseI}));
        save(rrFile,'reReg');
        rrInds{mouseI} = reReg;
        clear reReg;
    else
...
for mouseI = 1:numMice
    saveName = fullfile(dnmpFolder,mice{mouseI},'trialReliability.mat');
    reliability{mouseI} = load(saveName);
    
    dayUse{mouseI} = reliability{mouseI}.dayUse;
    clear reliability
    
    saveName = fullfile(ffFolder,mice{mouseI},'trialReliability.mat');
    reliability{mouseI} = load(saveName);
    
    dayUseFF{mouseI} = reliability{mouseI}.dayUse;
    clear reliability
end
...
disp('Loading stuff')
for mouseI = 1:numMice
    load(fullfile(dnmpFolder,mice{mouseI},'trialbytrial.mat'))
    ...
    load(fullfile(ffFolder,mice{mouseI},'trialbytrial.mat'))
    cellTBTff{mouseI} = trialbytrial;
    cellSSIff{mouseI} = sortedSessionInds;
    cellPresentff{mouseI} = cellSSI{mouseI} > 0;
    cellAllFilesff{mouseI} = allfiles;
    cellRealDaysff{mouseI} = realdays;
...
for mouseI = 1:numMice
    if exist(fullfile(mainFolder,mice{mouseI},'accuracy.mat'),'file') == 0
        ...
        performance = sessionAccuracyForcedUnforced(cellAllFiles{mouseI},'*Finalized.xlsx');
...
for mouseI = 1:numMice
    ...
    if exist(saveName,'file')==0
        [dayUse,~] = GetUseCells(cellTBT{mouseI}, lapPctThresh, consecLapThresh);
...
for mouseI = 1:numMice
    ...
    save(saveName,'dayUsePooled','threshAndConsecPooled','trialReliPooled')
    clear('dayUsePooled','threshAndConsecPooled','trialReliPooled')
    end
...
thisCellSplits = [];
for mouseI = 1:numMice
    shuffleDir = fullfile(dnmpFolder,mice{mouseI},splitDir);
...
thisCellSplitsFF = [];
for mouseI = 1:numMice
    shuffleDir = fullfile(ffFolder,mice{mouseI},splitDir);
    ...
    for stI = 1:length(splitterType)
        for slI = 1:length(splitterLoc)
            ...
            binsAboveShuffleFF{slI}{stI}{mouseI} = loadedSplit.binsAboveShuffle;
            thisCellSplitsFF{slI}{stI}{mouseI} = loadedSplit.thisCellSplits;
...
for mouseI = 1:numMice
    for slI = 1:1
        for stI = 1:length(splitterType)
            %Filter for active cells
            splitterCells{slI}{stI}{mouseI} = thisCellSplitsFF{slI}{stI}{mouseI}.*dayUseFilterFF{slI}{mouseI};
        ...
        [splittersLRonly{slI}{mouseI}, splittersSTonly{slI}{mouseI}, splittersBoth{slI}{mouseI},...
            splittersOne{slI}{mouseI}, splittersAny{slI}{mouseI}, splittersNone{slI}{mouseI}] = ...
            GetSplittingTypes(splittersLR{slI}{mouseI}, splittersST{slI}{mouseI}, dayUseFilterFF{slI}{mouseI});
            
        %Package into trait logicals
        traitGroupsFF{slI}{mouseI} = {logical(splittersLR{slI}{mouseI});... 
                                    logical(splittersST{slI}{mouseI});... 
                                    logical(splittersLRonly{slI}{mouseI});... 
                                    logical(splittersSTonly{slI}{mouseI}); ...
                                    logical(splittersBoth{slI}{mouseI}); ...
...
%% Splitters from DNMP to FF

%Verify cross-task rebuilt inds
for mouseI = 1:numMice
    traitAB = PoolTraitAcrossTasks(rrInds{mouseI}.sessionInds,rrInds{mouseI}.sessType,rrInds{mouseI}.sessInd,cellSSI{mouseI},cellSSIff{mouseI});
    
    load(fullfile(dnmpFolder,mice{mouseI},'fullReg'))
    allInds = fullReg.sessionInds;
    allInds(:,rrInds{mouseI}.skipSess)=0;
    isSame = allInds == traitAB;
    if sum(sum(isSame)) == (size(isSame,1)*size(isSame,2))
        disp(['remade cross task SSI for mouse ' num2str(mouseI) ' worked'])
    end
end

% Rebuild splitters across tasks
for mouseI = 1:numMice
     rrDayUse{mouseI} =  PoolTraitAcrossTasks(rrInds{mouseI}.sessionInds,rrInds{mouseI}.sessType,rrInds{mouseI}.sessInd,...
        dayUse{mouseI},dayUseFF{mouseI});
    for tgI = 1:numTraitGroups
        rrTG{mouseI}{tgI} = PoolTraitAcrossTasks(rrInds{mouseI}.sessionInds,rrInds{mouseI}.sessType,rrInds{mouseI}.sessInd,...
            traitGroups{1}{mouseI}{tgI},traitGroupsFF{1}{mouseI}{tgI});
    end
end


% Get day pairs for adjacent FF/DNMP
%{
for mouseI = 3:numMice
    dayTs = cell2mat(cellfun(@(x) str2double(x(end-5:end)),rrInds{mouseI}.allSess,'UniformOutput',false));
    [dSorted,dOrder] = sort(dayTs);
    skip = rrInds{mouseI}.skipSess(dOrder);
    dOrder(skip) = []; 
    dSorted(skip) = [];
    type = rrInds{mouseI}.sessType(dOrder);
    viewHere = [dSorted,type(:)];
    opv('viewHere')
    ffSesses = find(type==2);
    
    ffDNMPdays{mouseI} = [];
    ffDNMPdps{mouseI} = [];
    for ffI = 1:length(ffSesses)
        fH = ffSesses(ffI);
        sessPair = str2double(input(['For FF sess ' num2str(fH) ', ' num2str(dSorted(fH)) ', enter index of DNMP sess to use (0 for skip this one)>>'],'s'));
        if sessPair ~= 0
            ffDNMPdays{mouseI} = [ffDNMPdays{mouseI}; dSorted(fH), dSorted(sessPair)];
            ffDNMPdps{mouseI} = [ffDNMPdps{mouseI}; find(dayTs==dSorted(fH)), find(dayTs==dSorted(sessPair))];
        end
    end
    clear('viewHere')
end
%}
ffDNMPdays{1} = [160823      160825; 160829      160830; 160902      160901];
ffDNMPdays{2} = [160823      160824; 160829      160830; 160902      160901];
ffDNMPdays{3} = [161014      161018; 161022      161021; 161024      161025; 161028      161027; 161030      161031; 161105      161103];
ffDNMPdays{4} = [180425      180426; 180429      180428; 180430      180501; 180504      180503; 180508      180509; 180514      180513];
                 
ffDNMPdps{1} = [9     7; 4     2; 5     3]; 
ffDNMPdps{2} = [14    13; 4     2; 5     3];
ffDNMPdps{3} = [20    17; 7     9; 5     3; 4     2; 6     8; 15    13];
ffDNMPdps{4} = [12    11; 6     7; 4     2; 5     3; 9    10; 18    16];

% Find cell types present across both
dpsCheck = ffDNMPdps;
thingsNow = [3 4 5 8];
allSharedPctsPooled = nan(length(thingsNow),length(thingsNow));
for mouseI = 1:numMice
    for dayPairI = 1:length(dpsCheck{mouseI})
        %Get num cells shared across days
        dpHere = dpsCheck{mouseI}(dayPairI,:);
        cellsShared = rrDayUse{mouseI}(:,dpHere(1)) & rrDayUse{mouseI}(:,dpHere(2));
        
        cellsSharedNum{mouseI}{dayPairI} = sum(cellsShared);
        for tnI = 1:length(thingsNow)
            for tnJ = 1:length(thingsNow)
                ffTrait = rrTG{mouseI}{thingsNow(tnI)}(:,dpHere(1));
                dnmpTrait = rrTG{mouseI}{thingsNow(tnJ)}(:,dpHere(2));
                
                sharedTrait = ffTrait & dnmpTrait;

                sharedCount{mouseI}{dayPairI}(tnI,tnJ) = sum(sharedTrait);
            end
            
            sharedPcts{mouseI}{dayPairI}(tnI,:) = sharedCount{mouseI}{dayPairI}(tnI,:) / sum(sharedCount{mouseI}{dayPairI}(tnI,:));
        end
        
        %sharedPcts{mouseI}{dayPairI} = sharedCount{mouseI}{dayPairI} / cellsSharedNum{mouseI}{dayPairI};
        
        if cellsSharedNum{mouseI}{dayPairI} > 0
        allSharedPctsPooled(:,:,size(allSharedPctsPooled,3)+1) = sharedPcts{mouseI}{dayPairI};
        end
    end
end
allSharedPctsPooled(:,:,1) = [];

c = categorical({'Turn','Phase','Conjunctive'});
statsOut = []; sps = [];
scs = [ colorAssc{1}; colorAssc{2}; colorAssc{5}; 0.6 0.6 0.6;];

    figure('Position',[470 342 913 394]); %[729 513 497 340]
    for ccI = 1:length(thingsNow)
        sps{ccI} = subplot(1,length(thingsNow),ccI);
        [statsOut{slI}{ccI}] = PlotBarWithData(squeeze(allSharedPctsPooled(ccI,:,:))',scs,true,'jitter',[]);
        ylim([0 1.01])
