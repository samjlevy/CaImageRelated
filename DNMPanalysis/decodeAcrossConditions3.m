function [training, testing, decoded, postProbs] = decodeAcrossConditions3(trialbytrial, condsUse, typePredict, trainSess,...
...
numTestingLaps = length(testingLaps(condsUse(tcJ)).lapNums{testSess})
for tcJ = 1:length(condsUse)
    for lapI = 1:length(testingLaps
        trainLaps = trainingLaps(condsUse(tcJ))
...
training = []; testing = [];
for testCondJ = 1:length(condsUse) 
    testCondI = condsUse(testCondJ);
    otherConds = condsUse(condsUse~=testCondI);
