function [Corrs, numCellsUsed, dayPairs, condPairs] = PopVectorCorrs2TMaps(TMapA, TMapB, traitLogical, cellsUseOption, corrType, condPairs, dayPairs)
...
numConds = size(TMapA, 3);

disp('Auto assignment of condPairs not working now')
%disp('Right now only does the comparison within a single day')
...
for dpI = 1:numDayPairs
    for cpI = 1:numCondPairs
        ...
        if sum(cellsUse) > 1
        TRatesA = cell2mat(TMapA(cellsUse,dayPairs(dpI,1),condPairs(cpI,1)));
        TRatesB = cell2mat(TMapB(cellsUse,dayPairs(dpI,2),condPairs(cpI,2)));
        for binI = 1:numBins
