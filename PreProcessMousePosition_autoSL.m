function [xpos_interp,ypos_interp,start_time,MoMtime] = PreProcessMousePosition_autoSL(filepath, auto_thresh,varargin)
...
Ypix = pos_data(:,4);
Xpix(isnan(Xpix))=0;
Ypix(isnan(Ypix))=0;% y position in pixels (can be adjusted to cm)
catch
...
if exist(fullfile(pwd,'Pos_temp.mat'),'file') || exist(fullfile(pwd,'Pos.mat'),'file')
    ...
    if exist(fullfile(pwd,'Pos_temp.mat'),'file') && ~exist(fullfile(pwd,'Pos.mat'),'file')
       ...
       % doneCorrecting = input('Keep correcting (0) or use as is (1)?');
    elseif exist(fullfile(pwd,'Pos.mat'),'file')
...
%disp: optional exclude region
userect=input('Draw exclude region?','s');
if strcmpi(userect,'y')
    disp('Draw exclusion rectangle')
    obj.currentTime=1;
    b=readFrame(obj);
    figure(667)
    gcf;
    ...
    rect=getrect(666);
    close(667)
    inRect=Xpix > rect(1) & Xpix < rect(1)+rect(3) & Ypix > rect(2) & Ypix < rect(2)+rect(4);
end
% start auto-correction of anything above threshold
if exist('rect','var')
    auto_frames = (inRect | Xpix == 0 | Ypix == 0 | vel_init > auto_vel_thresh) & time > MoMtime; % & time > edit_end_time;
else    
...
%         display(['Time is ' num2str(time(sFrame+i*2)) ' seconds. Click the mouse''s back']);
        ...
        if button==1
            xAVI(sFrame+i*2) = xm;
            yAVI(sFrame+i*2) = ym;   
            Xpix(sFrame+i*2) = ceil(xAVI(sFrame+i*2)/0.6246);
            Ypix(sFrame+i*2) = ceil(yAVI(sFrame+i*2)/0.6246);
        end 
        % interpolate and apply correct position for previous frame
        xAVI(sFrame+i*2-1) = xAVI(sFrame+i*2-2)+(xAVI(sFrame+i*2)-xAVI(sFrame+i*2-2))/2;
        yAVI(sFrame+i*2-1) = yAVI(sFrame+i*2-2)+(yAVI(sFrame+i*2)-yAVI(sFrame+i*2-2))/2;
        Xpix(sFrame+i*2-1) = ceil(xAVI(sFrame+i*2-1)/0.6246);
        Ypix(sFrame+i*2-1) = ceil(yAVI(sFrame+i*2-1)/0.6246);
        
        
        % plot marker
        plot(xAVI(sFrame+i*2),yAVI(sFrame+i*2),marker{marker_fr(i)},'MarkerSize',4,'MarkerFaceColor',marker_face{marker_fr(i)});hold off;
    ...
    if length(vel)+1 == length(time)
        vel(length(vel)+1)=vel(end);
        plot(time(MouseOnMazeFrame:end),vel(MouseOnMazeFrame:end));
    else
        %plot(time(MouseOnMazeFrame:end-1),vel(MouseOnMazeFrame:end));
    end
    
    if auto_thresh_flag == 1 %&& length(ind_red)==length(time) && length(ind_red)==length(vel)
    ...
    % NRK edit
    save Pos_temp.mat Xpix Ypix xAVI yAVI MoMtime MouseOnMazeFrame edit_end_time
